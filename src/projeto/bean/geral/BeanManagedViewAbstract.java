...
public abstract class BeanManagedViewAbstract extends BeanReportView {
	...
	private Object getQueryConsulta() throws Exception {
		...
		if(condicaoPesquisaSelecionado.name().equalsIgnoreCase(CondicaoPesquisa.IGUAL_A.name())) {
			sql.append(" = retira_acentos(upper('");
			sql.append(valorPesquisa);
			sql.append("'))");
		}else if(condicaoPesquisaSelecionado.name().equalsIgnoreCase(CondicaoPesquisa.CONTEM.name())) {
				sql.append(" like retira_acentos(upper('%");
				sql.append(valorPesquisa);
				sql.append("%'))");
				
		}else if(condicaoPesquisaSelecionado.name().equalsIgnoreCase(CondicaoPesquisa.TERMINA_COM.name())) {
					sql.append(" like retira_acentos(upper('%");
					sql.append(valorPesquisa);
					sql.append("'))");
